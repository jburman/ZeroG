    ...
    public class LocalObjectServiceClientTest
    {
        ...
        public void CompressedTest()
        {
            using (var svc = new ObjectService(ObjectTestHelper.GetConfig()))
            {
                ...
                var objId1 = client.StoreCompressed(val1.ToByteArray());
                var objId2 = client.StoreCompressed(key2, val2.ToByteArray());

                var getVal = client.GetCompressed(objId1.ID);
                Assert.IsNotNull(getVal);
                Assert.AreEqual(val1, new Guid(getVal));

                getVal = client.GetCompressedBySecondaryKey(key2);
                Assert.IsNotNull(getVal);
                Assert.AreEqual(val2, new Guid(getVal));
            }
        }
        ...
        public void RemoveTest()
        {
            using (var svc = new ObjectService(ObjectTestHelper.GetConfig()))
            {
                ...
                getVal = client.GetCompressed(objId2.ID);
                Assert.IsNotNull(getVal);
